...
def load_model():
    ...
    # Use SAR encoder instead of CLIP
    model_path = "MBZUAI/geochat-7B"
    sar_encoder_path = "BenchMarks_GeoChat/mars_base_sar_encoder_only.pth"  # SAR encoder weights
    
    model_name = get_model_name_from_path(model_path)
    log(f"Detected model name: {model_name}")
    
    # Set vision tower to SAR encoder path
    import os
    sar_encoder_abs_path = os.path.abspath(sar_encoder_path)
    if not os.path.exists(sar_encoder_abs_path):
        log(f"WARNING: SAR encoder not found at {sar_encoder_abs_path}")
        log("Falling back to CLIP encoder...")
        vision_tower_path = None
    else:
        vision_tower_path = sar_encoder_abs_path
        log(f"Using SAR encoder from {vision_tower_path}")
    ...
    # If using SAR encoder, override the vision tower
    if vision_tower_path and os.path.exists(vision_tower_path):
        log("Replacing vision tower with SAR encoder...")
        from geochat.model.multimodal_encoder.sar_encoder import SARVisionTower
        model.model.vision_tower = SARVisionTower(vision_tower_path)
...
# ===========================================================================
def run_one(record: dict, chat, conv_templates, use_sar_encoder=False) -> str:
    img_path = PATCHES_ROOT / record["image"]
    question, gt = extract_question_and_gt(record)
    
    # Debug: print sample info
    print(f"\n[DEBUG] Processing: {record.get('id', 'unknown')}")
    print(f"[DEBUG] Image path: {img_path}")
    print(f"[DEBUG] Question: {question}")
    print(f"[DEBUG] GT: {gt}")
    print(f"[DEBUG] Using SAR encoder: {use_sar_encoder}")
    ...
    if use_sar_encoder:
        # Load TIFF for SAR encoder (512x512) and pass as tensor to avoid RGB conversion
        import tifffile
        img_np = tifffile.imread(img_path)
        print(f"[DEBUG] Loaded TIFF shape: {img_np.shape}")
        
        img_tensor = torch.from_numpy(img_np).float()
        if img_tensor.ndim == 2:
            img_tensor = img_tensor.unsqueeze(0)  # [1, 512, 512]
        img_tensor = img_tensor.unsqueeze(0)  # [1, 1, 512, 512] for batch
        # Pass tensor directly to avoid RGB conversion in Chat.encode_img
        image = img_tensor
    else:
        # Load RGB for CLIP encoder
        image = Image.open(img_path).convert("RGB")
        print(f"[DEBUG] Loaded RGB image")
    ...
    # Clear cache after each inference to prevent OOM
    torch.cuda.empty_cache()
...
# Main benchmark loop
# ===========================================================================
def run_benchmark(records: list, chat, conv_templates, split_name: str, use_sar_encoder=False) -> list:
    results = []
    n = len(records)
    ...
    for idx, record in enumerate(records):
        ...
        t0 = time.time()
        try:
            pred    = run_one(record, chat, conv_templates, use_sar_encoder)
            ...
            error   = str(e)
            log(f"  benchmark ERROR on {rec_id}: {e}")
...
def main():
    ...
    # Check if SAR encoder is being used
    import os
    sar_encoder_path = os.path.abspath("BenchMarks_GeoChat/mars_base_sar_encoder_only.pth")
    use_sar_encoder = os.path.exists(sar_encoder_path)
    if use_sar_encoder:
        log(f"Using SAR encoder for benchmark")
    else:
        log(f"Using CLIP encoder for benchmark")
    ...
    for split in splits:
        ...
        if len(records) > 0:
            ...
            print(f"  ID: {records[0].get('id', 'unknown')}")
            print(f"  Image: {records[0].get('image', 'unknown')}")
            print(f"  Category: {records[0].get('category', 'unknown')}")
            print(f"  Conversations: {records[0].get('conversations', [])}")
        ...
        t0 = time.time()
        results = run_benchmark(records, chat, conv_templates, split, use_sar_encoder)
        elapsed = time.time() - t0
