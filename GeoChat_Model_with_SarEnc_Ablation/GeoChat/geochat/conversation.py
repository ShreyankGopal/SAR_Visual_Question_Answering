...
class Chat:
    ...
    def answer_prepare(self, conv, img_list, max_new_tokens=300, num_beams=1, min_length=1, top_p=0.9,
        ...
        generation_kwargs = dict(
            input_ids=embs,
            images=img_list[0] if len(img_list) > 0 else None,
    ...
    def answer(self, conv, img_list, **kargs):
        ...
        with torch.inference_mode():
            output_ids = self.model.generate(
                ...
                images=generation_kwargs['images'],
                do_sample=False,
                temperature=generation_kwargs['temperature'],
                max_new_tokens=generation_kwargs['max_new_tokens'],
                use_cache=generation_kwargs['use_cache'],
                stopping_criteria=generation_kwargs['stopping_criteria']
            )
        output_text = self.tokenizer.decode(output_ids[0, generation_kwargs['input_ids'].shape[1]:], skip_special_tokens=True).strip()
        return output_text
    ...
    def model_generate(self, *args, **kwargs):
        ...
        with torch.inference_mode():
            output = self.model.generate(kwargs['kwargs']['input_ids'],
                                         ...
                                         streamer=kwargs['kwargs'].get('streamer', None),
