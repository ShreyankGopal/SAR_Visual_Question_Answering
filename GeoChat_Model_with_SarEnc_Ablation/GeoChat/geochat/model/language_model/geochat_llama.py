...
class GeoChatLlamaForCausalLM(LlamaForCausalLM, GeoChatMetaForCausalLM):
    ...
    def prepare_inputs_for_generation(
        ...
        # In Transformers >=4.36, generate() passes a DynamicCache object
        # instead of a tuple for past_key_values. An empty DynamicCache is
        # truthy (it's a non-None object), so the old `if past_key_values:`
        # check would truncate the ENTIRE prompt to just the last token on
        # the very first generation step, producing garbage output.
        # Fix: check whether the cache actually has content.
        has_past = past_key_values is not None
        if has_past:
            if hasattr(past_key_values, 'get_seq_length'):
                # DynamicCache from newer transformers
                has_past = past_key_values.get_seq_length() > 0
            elif isinstance(past_key_values, (tuple, list)):
                # Old-style tuple-of-tuples cache
                has_past = len(past_key_values) > 0
            # else: some other truthy object, treat as having past
