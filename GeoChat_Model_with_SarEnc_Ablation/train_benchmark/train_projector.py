...
train_projector.py
==================
Train the MLP projector for SAR encoder ablation study.
...
Usage:
    python train_benchmark/train_projector.py --config train_benchmark/config.yaml
"""
...
class SARVLMDataset(torch.utils.data.Dataset):
    """Dataset for SAR VLM training with proper causal language modeling labels."""
...
def train_epoch(model, dataloader, optimizer, device, epoch, global_step, grad_acc_steps=1, save_every_batches=10000, log_file=None):
    """Train for one epoch with proper autoregressive cross-entropy loss."""
    model.train()
    total_loss = 0
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        # Keep SAR input in FP32 (encoder weights are FP32)
        ...
        with torch.cuda.amp.autocast(enabled=torch.cuda.is_available()):
            # Use GeoChat's forward method - it handles vision encoding internally
            outputs = model(
                input_ids=input_ids,
                attention_mask=attention_mask,
                labels=labels,
            ...
            # Cross-entropy loss from language modeling
        ...
        if (batch_idx + 1) % grad_acc_steps == 0 or (batch_idx + 1) == num_batches:
        ...
        if global_step % save_every_batches == 0:
            save_checkpoint(model, optimizer, epoch, global_step, log_file=log_file)
    
    avg_loss = total_loss / num_batches
    return avg_loss, global_step


def save_checkpoint(model, optimizer, epoch, global_step, output_dir='train_benchmark/checkpoints', log_file=None):
    """Save model checkpoint."""
    os.makedirs(output_dir, exist_ok=True)
    checkpoint_path = os.path.join(output_dir, f'projector_step{global_step}.pth')
    
    # Save only the projector weights
    torch.save({
        'projector_state_dict': model.model.mm_projector.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
        'epoch': epoch,
        'global_step': global_step,
    }, checkpoint_path)
    
    msg = f"Saved checkpoint: {checkpoint_path}"
    print(msg)
    if log_file:
...
def main():
    ...
    if config['projector_type'] == 'sar_mlp':
        ...
        model.model.mm_projector.to(device=device, dtype=torch.float32)  # FP32 for training
    ...
    trainable_params = [p for p in model.model.mm_projector.parameters() if p.requires_grad]
    log_msg(f"Trainable parameters: {sum(p.numel() for p in trainable_params)}")
    ...
    dataset = SARVLMDataset(config['train_data'], config['data_root'], tokenizer, max_length=max_length)
    ...
    log_msg(f"Dataset size: {len(dataset)}")
    
    # Training loop
    global_step = 0
    grad_acc_steps = config.get('gradient_accumulation_steps', 1)
    save_every_batches = config.get('save_every_batches', 10000)
    
    log_msg("Starting training...")
    for epoch in range(config['num_epochs']):
        log_msg(f"--- Epoch {epoch + 1}/{config['num_epochs']} ---")
        avg_loss, global_step = train_epoch(
            model, dataloader, optimizer, device, epoch + 1, 
            global_step, grad_acc_steps, save_every_batches, log_file
        )
        log_msg(f"Epoch {epoch + 1} completed. Average loss: {avg_loss:.4f}")
    
    # Save final checkpoint
    save_checkpoint(model, optimizer, config['num_epochs'], global_step, log_file=log_file)
    log_msg("Training completed!")
    
    # Run validation if configured
    if config['run_validation_after_training']:
        log_msg("Running validation...")
        os.system(f"python train_benchmark/validate_projector.py --config {args.config}")
